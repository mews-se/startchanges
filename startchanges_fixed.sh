...
# Description: Return newest matching backup file for a target path
###############################################################################
find_latest_backup() {
    local target_file="$1"

    find "$(dirname -- "$target_file")" -maxdepth 1 -type f         -name "$(basename -- "$target_file").bak_*"         -printf '%T@ %p\n' 2>/dev/null |         sort -nr | head -n1 | cut -d' ' -f2-
}
...
restore_from_backup() {
    ...
    latest_backup="$(find_latest_backup "$target_file")"
